...
A `# yaml-language-server` directive at the top of `generators.yml` enables validation and autocompletion in editors that use the [YAML Language Server](https://github.com/redhat-developer/yaml-language-server), such as VS Code with the [YAML extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml), Neovim, and Emacs. IntelliJ ignores the directive; map `https://schema.buildwithfern.dev/generators-yml.json` to `generators.yml` under **Settings → Languages & Frameworks → Schemas and DTDs → JSON Schema Mappings** instead.
...
<ParamField path="auth" type="string | object" toc={true}>
  ...
  <Accordion title="Per-endpoint routing (endpoint-security)">
...
<ParamField path="headers" type="string or list of objects" toc={true}>
  ...
  - `client-default`: the value the generated SDK sends when neither an explicit value nor `env` supplies one. This is the equivalent of [`x-fern-default`](/learn/api-definitions/openapi/extensions/default-values) on an OpenAPI global header.
  - `default`: a documentation-only value displayed in the API Reference. It has no effect on generated SDKs.

  Generated SDKs resolve a [global header](/learn/sdks/deep-dives/global-headers) value in a fixed order: an option set on a single request, then the value passed to the client constructor, then the `env` environment variable, then `client-default`.
  ...
  <Accordion title="Object configuration">
...
<ParamField path="environments" type="object" toc={true}>
  ...
  Set `base-url-env` to the name of an environment variable that generated SDKs read for their [base URL](/learn/api-definitions/ferndef/api-yml/environments#base-url-from-an-environment-variable) when no base URL or environment is passed to the client. This value takes precedence over `x-fern-base-url-env` in the OpenAPI spec; overriding `environments` without setting `base-url-env` keeps the spec's value. Only the TypeScript SDK generator supports this setting.

  ```yaml title="generators.yml"
  api:
    environments:
      Production: "https://api.plants.com"
      Staging: "https://api.staging.plants.com"
      base-url-env: PLANTS_API_BASE_URL
  ```
</ParamField>
...
<ParamField path="settings.auto-generate-idempotency-key" type="boolean | object" default="false" toc={true}>
...
<ParamField path="namespace" type="string" toc={true}>
  Namespace for the specification. Useful for configuring a [single package with multiple API versions](/learn/api-definitions/overview/project-structure#combined-sdks-from-multiple-apis).  
</ParamField>
...
Use `image` instead of `name` to pull the generator from a [custom container registry](/learn/sdks/deep-dives/self-hosted#private-registry-setup) during [self-hosted generation](/learn/sdks/deep-dives/self-hosted) (remote generation doesn't support custom registries). Mutually exclusive with `name`.
...
<CardGroup cols={3}>
<Card title="TypeScript" icon={<img src="../images/icons/ts-light.svg" alt="TypeScript" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/typescript/configuration">
</Card>
<Card title="Python" icon={<img src="../images/icons/python-light.svg" alt="Python" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/python/configuration">
</Card>
<Card title="Go" icon={<img src="../images/icons/go-light.svg" alt="Go" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/go/configuration">
</Card>
<Card title="Java" icon={<img src="../images/icons/java-light.svg" alt="Java" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/java/configuration">
</Card>
<Card title=".NET" icon={<img src="../images/icons/csharp-light.svg" alt=".NET" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/csharp/configuration">
</Card>
<Card title="PHP" icon={<img src="../images/icons/php-light.svg" alt="PHP" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/php/configuration">
</Card>
<Card title="Ruby" icon={<img src="../images/icons/ruby-light.svg" alt="Ruby" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/ruby/configuration">
</Card>
<Card title="Swift" icon={<img src="../images/icons/swift-light.svg" alt="Swift" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/swift/configuration">
</Card>
<Card title="Rust" icon={<img src="../images/icons/rust-light.svg" alt="Rust" className="h-6 w-6" noZoom />} href="/learn/sdks/generators/rust/configuration">
</Card>
</CardGroup>
...
Designate the `mode` to specify how Fern handles your code changes. For cloud generation, specify the GitHub repository using `repository`. For [self-hosted generation](/learn/sdks/deep-dives/self-hosted), use `uri` and `token` instead. Fern doesn't push SDKs to GitLab or other providers; to host elsewhere, generate to your local file system and commit the output from your own pipeline.
...
The following option applies to every `github` mode, including self-hosted generation.

<ParamField path="workflows" type="boolean" default="true" required={false} toc={true}>
Whether generated `.github/workflows/*` files are included in the commit. Set to `false` when the GitHub App or token used for generation lacks the `workflows` permission: GitHub rejects any commit from such a token that adds or modifies workflow files, and `.fernignore` doesn't protect files that don't exist yet in the repository. With `workflows: false`, newly generated workflow files are dropped and tracked workflow files keep their current contents. All other generated files are committed as usual.
