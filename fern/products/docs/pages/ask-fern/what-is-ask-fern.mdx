...
The Dashboard also reports a **resolution rate** over the last week, month, or year - the percentage of conversations where Ask Fern returned a cited response.

Resolution is judged by an LLM. Once a conversation has at least one assistant reply and has been idle for 10 minutes, a background job sends the transcript to a model that marks it **Resolved** if the user received a cited response to their question, or **Unresolved** if the assistant couldn't provide a cited answer to all requests in the conversation. Conversations that haven't been scored yet show as **Pending** and aren't counted in the resolution rate.
</Accordion>
...
<Info>
  The [`ai-search` key](/learn/docs/configuration/site-level-settings#ask-fern-configuration) configures Ask Fern once it's enabled. It doesn't control whether or not Ask Fern is turned on for a site.
</Info>

To keep Ask Fern turned on but hide its page action button, set [`ask-ai`](/learn/docs/configuration/site-level-settings#page-actionsoptionsask-ai) to `false`:
