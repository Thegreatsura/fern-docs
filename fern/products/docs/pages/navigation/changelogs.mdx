...
On a site that uses [products](/learn/docs/navigation/products), a top-level `changelog` key in `docs.yml` adds a single changelog shared by every product. It's served at `/changelog`, with no product slug prefix, and doesn't appear in the product switcher.
...
The top-level `changelog` key accepts the same options as a changelog inside `navigation`, such as `title`, `slug`, and `icon`. It requires `products`; `fern check` rejects it on a site without them.
...
Set the default layout for all changelogs [in `docs.yml`](/learn/docs/configuration/site-level-settings#layoutchangelog-layout):
