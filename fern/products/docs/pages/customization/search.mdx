...
<Note>
`prioritize-current-product` applies to keyword search only — [Ask Fern](/learn/docs/ai-features/ask-fern/overview) does not receive the per-product ranking boost. `default-filter-by-current-product` scopes both keyword search and Ask Fern retrieval: when enabled, Ask AI questions are filtered to the reader's current product and version.
</Note>
...
<AccordionGroup>
  <Accordion title="Attribute weighting">
    ...
    | 9 | Description | The page's [meta description](/learn/docs/configuration/page-level-settings#subtitle-and-description) |
